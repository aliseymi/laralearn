@component('admin.layouts.content',['title' => 'ویرایش نظر'])
    @slot('breadcrumb')
        ...
        <li class="breadcrumb-item"><a href="{{ route('admin.comments.index') }}">لیست نظرات تایید شده</a></li>
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="card card-info">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" action="{{ route('admin.comments.update.comment',$comment) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="card-body d-flex flex-wrap">
                        <div class="form-group col-lg-12">
                            <label for="comment" class="col-sm-2 control-label">متن نظر</label>

                            <textarea type="text" name="comment" class="form-control"
                                      id="comment" placeholder="متن نظر را وارد کنید">{{ old('comment',$comment->comment) }}</textarea>
                        </div>
                    </div>
                    <!-- /.card-body -->
